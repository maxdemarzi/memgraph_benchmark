...
class Q16 extends Simulation {
  ...
  val query = """MATCH (s:User {id: $id})-[*1..2]->(n:User) WHERE n.age >= 18 RETURN DISTINCT n.id, n"""
  val cypherQuery: String = """{"statements" : [{"statement" : "%s", "parameters" : { "id": #{id} } }]}""".format(query)
  val queryName = "Q16 - neighbours_2_with_data_and_filter"
